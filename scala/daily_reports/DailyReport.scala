/**
 * Created by Igor Shekhtman on 6/26/2014.
 */
...
val PROCESS_ALL_QUERIES=bool(1)

// Send report emails and archive report html file
val DEBUG_MODE=bool(0)

// ============================ INITIALIZING GLOBAL VARIABLES VALUES =====================================================================================================

var TEST_TYPE="SanityTest"
var REPORT_TYPE="Daily engineering QA"
var LOGTYPE = "epoch"
var SENDER="donotreply@apixio.com"
var REPORT=""

val PASSED="<table><tr><td bgcolor='#00A303' align='center' width='800'><font size='3' color='white'><b>STATUS - PASSED</b></font></td></tr></table>"
val FAILED="<table><tr><td bgcolor='#DF1000' align='center' width='800'><font size='3' color='white'><b>STATUS - FAILED</b></font></td></tr></table>"
val SUBHDR="<table><tr><td bgcolor='#4E4E4E' align='left' width='800'><font size='3' color='white'><b>&nbsp;&nbsp;%s</b></font></td></tr></table>"


//CUR_TIME=strftime("%m/%d/%Y %H:%M:%S", gmtime())
var CUR_TIME=Calendar.getInstance().getTime()
...
val monthFormat = new SimpleDateFormat("M")
val yearFormat = new SimpleDateFormat("y")
//CURDAY=("%d", gmtime())
var CURDAY = dayFormat.format(today)
var CURMONTH = monthFormat.format(today)
var CURYEAR = monthFormat.format(today)
...
var DOCUMENTS_TRANSMITTED=20
var DOCUMENTS_TO_OCR=0
var DOCUMENTS_TO_PERSIST=0
var TAGED_TO_OCR=0
var TAGED_TO_PERSIST=0
var TAGGED_TOTAL=0
var QUERY_DESC=""
var COMPONENT_STATUS="PASSED"
var ORGID="N/A"
var BATCHID="N/A"
var USERNAME="N/A"
var UPLOADED_DR = 0
...
def checkEnvironmentandReceivers() {
  ...
  var RECEIVERS2 = ""
  var HTML_RECEIVERS = ""
  //var ENVIRONMENT, USERNAME, ORGID, PASSWORD, HOST
  // Environment for SanityTest is passed as a paramater. Staging is a default value
  ...
  if (sys.argv.length() < 2) {
    ...
    ENVIRONMENT = sys.argv{1}.toString
  }
  ...
  if (sys.argv.length() > 2) {
    RECEIVERS = sys.argv{2}.toString
    RECEIVERS2 = sys.argv{3}.toString
    HTML_RECEIVERS = """To: Eng <%s>,Ops <%s>\n""" % (sys.argv{2}.toString, sys.argv{3}.toString)
  }
  else if((sys.argv.length() < 3) or DEBUG_MODE) {
...
def test(debug_type:Integer, debug_msg:String) {
...
def writeReportHeader () {
  ...
  REPORT = REPORT + """Enviromnent: <b><font color='red'>%s%s</font></b><br><br>\n""" % (ENVIRONMENT.substr(0,1).toUpperCase(), ENVIRONMENT.substring(2).toLowerCase())
...
def obtainFailedJobs(table:String){
...
REPORT = REPORT+"<tr>" +
  ...
  "<td>"+i{1}.toString()+"&nbsp;&nbsp;</td>" +
  "<td>"+i{2}.toString()+"&nbsp;&nbsp;</td>"
if (ORGMAP.contains(i{3}.toString)) {
    REPORT = REPORT + "<td>"+ORGMAP{(i{3}).toString}+" ("+(i{3}.toString)+")</td>"
}
else
  {
    REPORT = REPORT + "<td>" + i{3}.toString +" (" + i{3}.toString + ")</td>"
...
def obtainErrors(activity:String, summary_table_name:String, unique_id:Integer) {
  //global REPORT, cur, conn
  //global DAY, MONTH, COMPONENT_STATUS
  ...
  REPORT = REPORT + "<td bgcolor='#FFFF00'>" + i{0}.toString +"</td>"
  if (ORGMAP.contains(i{1}.toString)) {
    REPORT = REPORT + "<td bgcolor='#FFFF00'>" + ORGMAP(i{1}.toString) +" (" + i{1}.toString +")</td></tr>"
  }
  else {
    REPORT = REPORT + "<td bgcolor='#FFFF00'>" + i{1}.toString +" (" + i{1}.toString +")</td></tr>"
    REPORT = REPORT + "<tr><td colspan='4' bgcolor='#FFFF00'>Error: <i>" + i{2}.toString +"</i></td></tr>"
    COMPONENT_STATUS = "FAILED"
  }
  if (ROW == 0) {
...
def removeHtmlTags(data:String) {
  //p = re.compile(r '<.*?> ')
...
def careOptimizerErrors(table:String) {
  //global REPORT, cur, conn
  //global DAY, MONTH, COMPONENT_STATUS
  //============================================================================
  //================= LIST OF CARE OPTIMIZER ERRORS ============================
  //============================================================================
  val major_error_list = List("Unexpected error while preparing query ",
    "Patient not found ")
  //============================================================================
  //============================================================================
  //============================================================================
  ...
  if (major_error_list.exists(i{0}.toString)){
  //if any(i{0}.toString )[: 15] ) in s for s in major_error_list)
    REPORT = REPORT + "<tr><td bgcolor='#FFFF00'>" + i{3}.toString +"</td><td bgcolor='#FFFF00'>" + removeHtmlTags(i{0}.toString) +"</td><td bgcolor='#FFFF00'>" + i{1}.toString +"</td><td bgcolor='#FFFF00'>" + i{2}.toString +"</td></tr>"
  }
  else
  {
    REPORT = REPORT + "<tr><td>" + i{3}.toString +"</td><td>" + removeHtmlTags(i{0}.toString) +"</td><td>" + i{1}.toString +"</td><td>" + i{2}.toString +"</td></tr>"
    COMPONENT_STATUS = "FAILED"
  }
  if (ROW == 0) {
...
def careOptimizerLoad(table:String) {
  //global REPORT, cur, conn
  //global DAY, MONTH, COMPONENT_STATUS
  ...
  print i
  REPORT = REPORT + "<tr><td>" + i{0}.toString +"</td>"
...
def careOptimizerSearch(table:String) {
  //global REPORT, cur, conn
  //global DAY, MONTH, COMPONENT_STATUS
...
def summaryLogstrafficTotals(table:String) {
  //global REPORT, cur, conn
  //global DAY, MONTH, COMPONENT_STATUS
...
def uploadSummary(activity:String, summary_table_name:String, unique_id:Integer) {
  //global REPORT, cur, conn
  //global DAY, MONTH, COMPONENT_STATUS
...
def archiveReport() {
  ...
  if (not DEBUG_MODE) {
    ...
    if (!BACKUPREPORTFOLDER.exists()) {
      BACKUPREPORTFOLDER.mkdir()
    }
    ...
    REPORTXTSTRING = "Daily " + ENVIRONMENT.head.toUpperCase() + ENVIRONMENT.tail.toLowerCase() + " Report - " + MONTH_FMN.toString + " " + DAY.toString + ", " + YEAR.toString + "\t" + "reports/" + ENVIRONMENT + "/pipeline/" + str(YEAR) + "/" + str(MONTH) + "/" + REPORTFILENAME + "\n"
