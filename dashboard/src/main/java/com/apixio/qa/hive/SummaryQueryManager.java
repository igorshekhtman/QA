...
public class SummaryQueryManager {
	...
	public SummaryQueryManager() {
		...
		coordinatorStats.setQuery("select get_json_object(line, '$.datestamp') as time, get_json_object(line, '$.coordinator.stats') as stats_json, month, day from production_logs_coordinator_epoch where get_json_object(line, '$.coordinator.stats.parser.queuedCount') is not null");
