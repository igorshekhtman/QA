...
def initializeGlobalArrays():
	global REQUESTS_SUCCEEDED, REQUESTS_FAILED, B_REQUESTS_SUCCEEDED, B_REQUESTS_FAILED
	REQUESTS_SUCCEEDED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		REQUESTS_SUCCEEDED[i] = 0
	REQUESTS_FAILED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		REQUESTS_FAILED[i] = 0
	B_REQUESTS_SUCCEEDED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		B_REQUESTS_SUCCEEDED[i] = 0
	B_REQUESTS_FAILED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		B_REQUESTS_FAILED[i] = 0	
...
def runBaseRequestsFailed(environment):
	global B_REQUESTS_FAILED
	#LOGFILE = selectLogFile(environment,"parser")
	LOGFILE = "summary_useraccount_request_staging"
	
	
	print ("Running %s Hive Query to extract failed jobs baseline data, please wait ...\n") % (environment)

		
	cur.execute("""SELECT COUNT(*) as total_requests, \
		year, month, day \
		FROM %s \
	...
	#print new_result
		B_REQUESTS_FAILED[ind] = int(i[0])
		ind = ind + 1
	
	
def runBaseRequestsSucceeded(environment):
	global B_REQUESTS_SUCCEEDED
	#LOGFILE = selectLogFile(environment,"parser")
	LOGFILE = "summary_useraccount_request_staging"
	...
	for i in new_result:
		...
		#print i
		#print cur.fetch()
		B_REQUESTS_SUCCEEDED[ind] = int(i[0])
		ind = ind + 1	
	
	
	
def runRequestsSucceeded(environment):
	...
	for i in new_result:
		...
		#print i
		#print cur.fetch()
		REQUESTS_SUCCEEDED[ind] = int(i[0])
		ind = ind + 1	

def runRequestsFailed(environment):
	...
	for i in new_result:
		print (str(i[0])+" \t\t "+str(i[1])+" \t\t "+str(i[2])+" \t\t "+str(i[3]))	
	#print new_result
		REQUESTS_FAILED[ind] = int(i[0])
		ind = ind + 1
	
def runQueries(environment):
	global ST_DAY, EN_DAY, MONTH, YEAR, REQUESTS_SUCCEEDED, ORGID, DAYSOFMONTH, REQUESTS_FAILED, B_REQUESTS_FAILED, B_REQUESTS_SUCCEEDED
	global ST_DAY, ST_MONTH, ST_MONTHS,ST_YEAR, EN_DAY, EN_MONTH, EN_MONTHS, EN_YEAR
	
	runRequestsSucceeded(environment)
	runRequestsFailed(environment)	
	runBaseRequestsSucceeded(environment)
	runBaseRequestsFailed(environment)
	#time.sleep(45)	
...
def appendData():
	global DATA, ORGID, ENVIRONMENT, MONTH_FMN#, REQUESTS_FAILED, REQUESTS_SUCCEEDED, B_REQUESTS_FAILED, B_REQUESTS_SUCCEEDED
	global ST_DAY, ST_MONTH, ST_MONTHS,ST_YEAR, EN_DAY, EN_MONTH, EN_MONTHS, EN_YEAR
	
	JSON_DATA_FILENAME ="/var/www/html/json/"+ENVIRONMENT.lower()+".user.accounts.requests.per.day.json" 
	print ("Start writing json data ...\n")

	new_json_data = { \
		"charttitle": ""+ENVIRONMENT.lower()+".user.accounts.requests.per.day", \
		"charttype": "line", \
		"dotColor": "black", \
		"dotSize": "4", \
		"updateInterval": "5", \
		"legendtitle": {"1": "Legend"}, \
		"legenddata": { "1": "Succeeded Requests", "2": "Failed Requests" }, \
		"legendcolors": { "1": "lime", "2": "red" }, \
		"xaxistitle": str(DAYSBACK+1)+" Days", \
		"baselinedata": { \
		"1": B_REQUESTS_SUCCEEDED, \
		"2": B_REQUESTS_FAILED}, \
		"xaxisdata": { \
		"1": calcXAxis()}, \
		"yaxistitle": "# of Requests",
		"yaxisdata": { \
		"1": REQUESTS_SUCCEEDED, \
        "2": REQUESTS_FAILED} \
		}	
