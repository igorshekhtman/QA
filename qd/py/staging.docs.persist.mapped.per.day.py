...
def runBaseDocFailed(environment):
	...
	LOGFILE = "summary_persist_mapper_staging"
	
	print ("Running %s Hive Query to extract failed jobs baseline data, please wait ...\n") % (environment)


		
	cur.execute("""SELECT COUNT(DISTINCT doc_id) as  Persist_mapper_count, \
		year, month, day \
		FROM %s \
...
def appendData():
	...
	JSON_DATA_FILENAME ="/var/www/html/json/"+ENVIRONMENT.lower()+".docs.persist.mapped.per.day.json" 
	print ("Start writing json data ...\n")

	new_json_data = { \
		"charttitle": ""+ENVIRONMENT.lower()+".docs.persist.mapped.per.day", \
		"charttype": "line", \
		"dotColor": "black", \
		"dotSize": "4", \
		"updateInterval": "5", \
