...
def initializeGlobalArrays():
	global OPPORTUNITIES_SUCCEEDED, OPPORTUNITIES_FAILED, B_OPPORTUNITIES_SUCCEEDED, B_OPPORTUNITIES_FAILED
	OPPORTUNITIES_SUCCEEDED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		OPPORTUNITIES_SUCCEEDED[i] = 0
	OPPORTUNITIES_FAILED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		OPPORTUNITIES_FAILED[i] = 0
	B_OPPORTUNITIES_SUCCEEDED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		B_OPPORTUNITIES_SUCCEEDED[i] = 0
	B_OPPORTUNITIES_FAILED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		B_OPPORTUNITIES_FAILED[i] = 0	
...
def runBaseEventFailed(environment):
	global B_OPPORTUNITIES_FAILED
	LOGFILE = selectLogFile(environment,"bundler")
	...
	print ("Running %s Hive Query to extract failed events baseline data, please wait ...\n") % (environment)

		
	cur.execute("""SELECT SUM(get_json_object(line, '$.bundler.consume.generated_opportunities')) as total_generated_opportunities, \
		year, month, day \
		FROM %s \
	...
	print ("Ended running %s Hive Query to extract baseline failed events ...\n")	 % (environment)
	result = cur.fetch()
	...
	#print new_result
		B_OPPORTUNITIES_FAILED[ind] = int(i[0])
		ind = ind + 1
	
	
def runBaseEventSucceeded(environment):
	global B_OPPORTUNITIES_SUCCEEDED
	LOGFILE = selectLogFile(environment,"bundler")
	...
	print ("Running %s Hive Query to extract successful events baseline data, please wait ...\n") % (environment)
	...
	print ("Ended running %s Hive Query to extract baseline successful events ...\n")	 % (environment)
	...
	for i in new_result:
		...
		#print i
		#print cur.fetch()
		B_OPPORTUNITIES_SUCCEEDED[ind] = int(i[0])
		ind = ind + 1	
	
	
	
def runEventSucceeded(environment):
	global OPPORTUNITIES_SUCCEEDED
	LOGFILE = selectLogFile(environment,"bundler")
		
	print ("Running %s Hive Query to extract successful events, please wait ...\n") % (environment)
	...
	print ("Ended running %s Hive Query to extract successful events ...\n")	 % (environment)
	...
	for i in new_result:
		...
		#print i
		#print cur.fetch()
		OPPORTUNITIES_SUCCEEDED[ind] = int(i[0])
		ind = ind + 1	

def runEventFailed(environment):
	global OPPORTUNITIES_FAILED
	LOGFILE = selectLogFile(environment,"bundler")
	
	print ("Running %s Hive Query to extract failed events, please wait ...\n") % (environment)
	...
	print ("Ended running %s Hive Query to extract failed events ...\n")	 % (environment)
	result = cur.fetch()
	...
	for i in new_result:
		print (str(i[0])+" \t\t "+str(i[1])+" \t\t "+str(i[2])+" \t\t "+str(i[3]))	
	#print new_result
		OPPORTUNITIES_FAILED[ind] = int(i[0])
		ind = ind + 1
	
def runQueries(environment):
	global ST_DAY, EN_DAY, MONTH, YEAR, OPPORTUNITIES_SUCCEEDED, ORGID, DAYSOFMONTH, OPPORTUNITIES_FAILED, B_OPPORTUNITIES_FAILED, B_OPPORTUNITIES_SUCCEEDED
	global ST_DAY, ST_MONTH, ST_MONTHS,ST_YEAR, EN_DAY, EN_MONTH, EN_MONTHS, EN_YEAR
	
	runEventSucceeded(environment)
	runEventFailed(environment)	
	runBaseEventSucceeded(environment)
	runBaseEventFailed(environment)
	#time.sleep(45)	
...
def appendData():
	global DATA, ORGID, ENVIRONMENT, MONTH_FMN#, OPPORTUNITIES_FAILED, OPPORTUNITIES_SUCCEEDED, B_OPPORTUNITIES_FAILED, B_OPPORTUNITIES_SUCCEEDED
	global ST_DAY, ST_MONTH, ST_MONTHS,ST_YEAR, EN_DAY, EN_MONTH, EN_MONTHS, EN_YEAR
	
	JSON_DATA_FILENAME ="/var/www/html/json/"+ENVIRONMENT.lower()+".opportunities.after.bundling.json" 
	print ("Start writing json data ...\n")

	new_json_data = { \
		"charttitle": ""+ENVIRONMENT.lower()+".opportunities.after.bundling", \
		"charttype": "line", \
		"dotColor": "black", \
		"dotSize": "4", \
		"updateInterval": "5", \
		"legendtitle": {"1": "Legend"}, \
		"legenddata": { "1": "Succeeded Opportunities", "2": "Failed Opportunities" }, \
		"legendcolors": { "1": "lime", "2": "red" }, \
		"xaxistitle": str(DAYSBACK+1)+" Days", \
		"baselinedata": { \
		"1": B_OPPORTUNITIES_SUCCEEDED, \
		"2": B_OPPORTUNITIES_FAILED}, \
		"xaxisdata": { \
		"1": calcXAxis()}, \
		"yaxistitle": "# of Opportunities",
		"yaxisdata": { \
		"1": OPPORTUNITIES_SUCCEEDED, \
        "2": OPPORTUNITIES_FAILED} \
		}	
