...
def initializeGlobalArrays():
	global EVENTS_SUCCEEDED, EVENTS_FEILED, B_EVENTS_SUCCEEDED, B_EVENTS_FEILED
	EVENTS_SUCCEEDED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		EVENTS_SUCCEEDED[i] = 0
	EVENTS_FEILED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		EVENTS_FEILED[i] = 0
	B_EVENTS_SUCCEEDED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		B_EVENTS_SUCCEEDED[i] = 0
	B_EVENTS_FEILED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		B_EVENTS_FEILED[i] = 0	
...
def runBaseEventFailed(environment):
	...
	LOGFILE = "summary_event_reducer_staging"
	
	print ("Running %s Hive Query to extract failed events baseline data, please wait ...\n") % (environment)

		
	cur.execute("""SELECT COUNT(*) as Total_Reducer_Events_Count, \
		year, month, day \
		FROM %s \
...
def runBaseEventSucceeded(environment):
	...
	cur.execute("""SELECT SUM(num_of_events_persisted) as Total_Reducer_Events_Count, \
		year, month, day \
		FROM %s \
		WHERE year*10000+month*100+day >= %s and year*10000+month*100+day <= %s \
		and \
		num_of_events_persisted is not null and \
		status = 'success' \
		GROUP BY year, month, day ORDER BY year, month, day ASC""" % (LOGFILE, (START_DATE_BASE.year * 10000 + START_DATE_BASE.month * 100 + START_DATE_BASE.day), (END_DATE_BASE.year * 10000 + END_DATE_BASE.month * 100 + END_DATE_BASE.day)))
	...
	for i in new_result:
		...
		#print i
		#print cur.fetch()
		B_EVENTS_SUCCEEDED[ind] = int(i[0])
		ind = ind + 1	
...
def runEventFailed(environment):
	...
	for i in new_result:
		print (str(i[0])+" \t\t "+str(i[1])+" \t\t "+str(i[2])+" \t\t "+str(i[3]))	
	#print new_result
		EVENTS_FEILED[ind] = int(i[0])
		ind = ind + 1
	
def runQueries(environment):
	global ST_DAY, EN_DAY, MONTH, YEAR, EVENTS_SUCCEEDED, ORGID, DAYSOFMONTH, EVENTS_FEILED, B_EVENTS_FEILED, B_EVENTS_SUCCEEDED
	global ST_DAY, ST_MONTH, ST_MONTHS,ST_YEAR, EN_DAY, EN_MONTH, EN_MONTHS, EN_YEAR
...
def appendData():
	global DATA, ORGID, ENVIRONMENT, MONTH_FMN#, EVENTS_FEILED, EVENTS_SUCCEEDED, B_EVENTS_FEILED, B_EVENTS_SUCCEEDED
	global ST_DAY, ST_MONTH, ST_MONTHS,ST_YEAR, EN_DAY, EN_MONTH, EN_MONTHS, EN_YEAR
	
	JSON_DATA_FILENAME ="/var/www/html/json/"+ENVIRONMENT.lower()+".events.reduced.per.day.json" 
	print ("Start writing json data ...\n")

	new_json_data = { \
		"charttitle": ""+ENVIRONMENT.lower()+".events.reduced.per.day", \
		"charttype": "line", \
		"dotColor": "black", \
		"dotSize": "4", \
		"updateInterval": "5", \
		"legendtitle": {"1": "Legend"}, \
		"legenddata": { "1": "Succeeded Events", "2": "Failed Events" }, \
		"legendcolors": { "1": "lime", "2": "red" }, \
		"xaxistitle": str(DAYSBACK+1)+" Days", \
		"baselinedata": { \
		"1": B_EVENTS_SUCCEEDED, \
		"2": B_EVENTS_FEILED}, \
		"xaxisdata": { \
		"1": calcXAxis()}, \
		"yaxistitle": "# of Events",
		"yaxisdata": { \
		"1": EVENTS_SUCCEEDED, \
        "2": EVENTS_FEILED} \
		}	
