...
def initializeGlobalArrays():
	...
	JOBS_SUCCEEDED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		JOBS_SUCCEEDED[i] = 0
	JOBS_FAILED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		JOBS_FAILED[i] = 0
	B_JOBS_SUCCEEDED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		B_JOBS_SUCCEEDED[i] = 0
	B_JOBS_FAILED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		B_JOBS_FAILED[i] = 0	
...
def runBaseJobFailed(environment):
	...
	# cur.execute("""SELECT COUNT(DISTINCT(get_json_object(line, '$.job.jobID'))) as failed_count, \
		# year, month, day \
		# FROM %s \
		# WHERE \
		# ((day>=%s and month=%s) and (day<=%s and month=%s)) and \
		# year=%s and \
		# get_json_object(line, '$.job.status') = 'error' \
		# GROUP BY year, month, day ORDER BY year, month, day ASC""" % (LOGFILE, START_DATE_BASE.day, START_DATE_BASE.month, END_DATE_BASE.day, END_DATE_BASE.month, YEAR))	
	
		#(START_DATE_BASE.year * 10000 + START_DATE_BASE.month * 100 + START_DATE_BASE.day)
...
def runJobSucceeded(environment):
	...
	# cur.execute("""SELECT COUNT(DISTINCT(get_json_object(line, '$.job.jobID'))) as count, \
		...
		# year=%s and \
		# get_json_object(line, '$.job.status') = 'success' \
		# GROUP BY year, month, day ORDER BY year, month, day ASC""" % (LOGFILE, ST_DAY, ST_MONTH, EN_DAY, EN_MONTH, YEAR))	
...
def runQueries(environment):
	global ST_DAY, EN_DAY, MONTH, YEAR, JOBS_SUCCEEDED, ORGID, DAYSOFMONTH, JOBS_FAILED, B_JOBS_FAILED, B_JOBS_SUCCEEDED
	global ST_DAY, ST_MONTH, ST_MONTHS,ST_YEAR, EN_DAY, EN_MONTH, EN_MONTHS, EN_YEAR
...
def appendData():
	global DATA, ORGID, ENVIRONMENT, MONTH_FMN#, JOBS_FAILED, JOBS_SUCCEEDED, B_JOBS_FAILED, B_JOBS_SUCCEEDED
	global ST_DAY, ST_MONTH, ST_MONTHS,ST_YEAR, EN_DAY, EN_MONTH, EN_MONTHS, EN_YEAR
	
	JSON_DATA_FILENAME ="/var/www/html/json/"+ENVIRONMENT.lower()+".jobs.succeeded.per.day.json" 
	print ("Start writing json data ...\n")

	new_json_data = { \
		"charttitle": ""+ENVIRONMENT.lower()+".jobs.succeeded.or.failed.per.day", \
		"charttype": "line", \
		"dotColor": "black", \
		"dotSize": "4", \
		"updateInterval": "5", \
