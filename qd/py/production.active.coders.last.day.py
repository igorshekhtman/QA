...
def initializeGlobalArrays():
	global CODERS_SUCCEEDED, CODERS_FAILED, B_CODERS_SUCCEEDED, B_CODERS_FAILED
	CODERS_SUCCEEDED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		CODERS_SUCCEEDED[i] = 0
	CODERS_FAILED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		CODERS_FAILED[i] = 0
	B_CODERS_SUCCEEDED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		B_CODERS_SUCCEEDED[i] = 0
	B_CODERS_FAILED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		B_CODERS_FAILED[i] = 0	
...
def runBaseDocFailed(environment):
	...
	#print new_result
		B_CODERS_FAILED[ind] = int(i[0])
		ind = ind + 1
	
	
def runBaseDocSucceeded(environment):
	global B_CODERS_SUCCEEDED
	LOGFILE = selectLogFile(environment,"ocr")
	...
	for i in new_result:
		...
		#print i
		#print cur.fetch()
		B_CODERS_SUCCEEDED[ind] = int(i[0])
		ind = ind + 1	
	
	
	
def runDocSucceeded(environment):
	global CODERS_SUCCEEDED
	LOGFILE = selectLogFile(environment,"ocr")
	...
	for i in new_result:
		...
		#print i
		#print cur.fetch()
		CODERS_SUCCEEDED[ind] = int(i[0])
		ind = ind + 1	

def runDocFailed(environment):
	global CODERS_FAILED
	LOGFILE = selectLogFile(environment,"ocr")
	...
	for i in new_result:
		print (str(i[0])+" \t\t "+str(i[1])+" \t\t "+str(i[2])+" \t\t "+str(i[3]))	
	#print new_result
		CODERS_FAILED[ind] = int(i[0])
		ind = ind + 1
	
def runQueries(environment):
	global ST_DAY, EN_DAY, MONTH, YEAR, CODERS_SUCCEEDED, ORGID, DAYSOFMONTH, CODERS_FAILED, B_CODERS_FAILED, B_CODERS_SUCCEEDED
	global ST_DAY, ST_MONTH, ST_MONTHS,ST_YEAR, EN_DAY, EN_MONTH, EN_MONTHS, EN_YEAR
...
def appendData():
	global DATA, ORGID, ENVIRONMENT, MONTH_FMN#, CODERS_FAILED, CODERS_SUCCEEDED, B_CODERS_FAILED, B_CODERS_SUCCEEDED
	global ST_DAY, ST_MONTH, ST_MONTHS,ST_YEAR, EN_DAY, EN_MONTH, EN_MONTHS, EN_YEAR
	
	JSON_DATA_FILENAME ="/var/www/html/json/"+ENVIRONMENT.lower()+".active.coders.last.day.json" 
	print ("Start writing json data ...\n")

	new_json_data = { \
		"charttitle": ""+ENVIRONMENT.lower()+".active.coders.last.day", \
		"charttype": "line", \
		"dotColor": "black", \
		"dotSize": "4", \
		"updateInterval": "5", \
		"legendtitle": {"1": "Legend"}, \
		"legenddata": { "1": "Succeeded Coders", "2": "Failed Coders" }, \
		"legendcolors": { "1": "lime", "2": "red" }, \
		"xaxistitle": str(DAYSBACK+1)+" Days", \
		"baselinedata": { \
		"1": B_CODERS_SUCCEEDED, \
		"2": B_CODERS_FAILED}, \
		"xaxisdata": { \
		"1": calcXAxis()}, \
		"yaxistitle": "# of Active Coders",
		"yaxisdata": { \
		"1": CODERS_SUCCEEDED, \
        "2": CODERS_FAILED} \
		}	
