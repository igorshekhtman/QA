...
def runBaseEventFailed(environment):
	...
	cur.execute("""SELECT SUM(get_json_object(line, '$.bundler.consume.raw_events')) as total_raw_events, \
		year, month, day \
		FROM %s \
...
def appendData():
	...
	JSON_DATA_FILENAME ="/var/www/html/json/"+ENVIRONMENT.lower()+".patient.events.after.bundling.json" 
	print ("Start writing json data ...\n")

	new_json_data = { \
		"charttitle": ""+ENVIRONMENT.lower()+".patient.events.after.bundling", \
		"charttype": "line", \
		"dotColor": "black", \
		"dotSize": "4", \
		"updateInterval": "5", \
