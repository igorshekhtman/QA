...
def runBaseDocFailed(environment):
	...
	LOGFILE = "summary_docreceiver_archive_staging"
	
	print ("Running %s Hive Query to extract failed jobs baseline data, please wait ...\n") % (environment)


		
	cur.execute("""SELECT COUNT(DISTINCT doc_id) as documents_archived, \
		year, month, day \
		FROM %s \
...
def appendData():
	...
	JSON_DATA_FILENAME ="/var/www/html/json/"+ENVIRONMENT.lower()+".docs.archived.per.day.json" 
	print ("Start writing json data ...\n")

	new_json_data = { \
		"charttitle": ""+ENVIRONMENT.lower()+".docs.archived.per.day", \
		"charttype": "line", \
		"dotColor": "black", \
		"dotSize": "4", \
		"updateInterval": "5", \
