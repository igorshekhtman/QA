...
def initializeGlobalArrays():
	global ERRORS_SUCCEEDED, ERRORS_FAILED, B_ERRORS_SUCCEEDED, B_ERRORS_FAILED
	ERRORS_SUCCEEDED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		ERRORS_SUCCEEDED[i] = 0
	ERRORS_FAILED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		ERRORS_FAILED[i] = 0
	B_ERRORS_SUCCEEDED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		B_ERRORS_SUCCEEDED[i] = 0
	B_ERRORS_FAILED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		B_ERRORS_FAILED[i] = 0	
...
def runBaseErrorsFailed(environment):
	global B_ERRORS_FAILED
	...
	cur.execute("""SELECT COUNT(*) as total_errors, \
		year, month, day \
		FROM %s \
		WHERE year*10000+month*100+day >= %s and year*10000+month*100+day <= %s \
		and \
		get_json_object(line, '$.level') ='EVENT' and \
		(get_json_object(line, '$.status') = 'error' or get_json_object(line, '$.status') = 'apoError') \
	...
	#print new_result
		B_ERRORS_FAILED[ind] = int(i[0])
		ind = ind + 1
	
	
def runBaseDocSucceeded(environment):
	global B_ERRORS_SUCCEEDED
	LOGFILE = selectLogFile(environment,"event")
	...
	for i in new_result:
		...
		#print i
		#print cur.fetch()
		B_ERRORS_SUCCEEDED[ind] = int(i[0])
		ind = ind + 1	
	
	
	
def runDocSucceeded(environment):
	global ERRORS_SUCCEEDED
	LOGFILE = selectLogFile(environment,"event")
	...
	for i in new_result:
		...
		#print i
		#print cur.fetch()
		ERRORS_SUCCEEDED[ind] = int(i[0])
		ind = ind + 1	

def runErrorsFailed(environment):
	global ERRORS_FAILED
	LOGFILE = selectLogFile(environment,"event")
	...
	for i in new_result:
		print (str(i[0])+" \t\t "+str(i[1])+" \t\t "+str(i[2])+" \t\t "+str(i[3]))	
	#print new_result
		ERRORS_FAILED[ind] = int(i[0])
		ind = ind + 1
	
def runQueries(environment):
	global ST_DAY, EN_DAY, MONTH, YEAR, ERRORS_SUCCEEDED, ORGID, DAYSOFMONTH, ERRORS_FAILED, B_ERRORS_FAILED, B_ERRORS_SUCCEEDED
	global ST_DAY, ST_MONTH, ST_MONTHS,ST_YEAR, EN_DAY, EN_MONTH, EN_MONTHS, EN_YEAR
	
	#runDocSucceeded(environment)
	runErrorsFailed(environment)	
	#runBaseDocSucceeded(environment)
	runBaseErrorsFailed(environment)
	#time.sleep(45)	
...
def appendData():
	global DATA, ORGID, ENVIRONMENT, MONTH_FMN#, ERRORS_FAILED, ERRORS_SUCCEEDED, B_ERRORS_FAILED, B_ERRORS_SUCCEEDED
	global ST_DAY, ST_MONTH, ST_MONTHS,ST_YEAR, EN_DAY, EN_MONTH, EN_MONTHS, EN_YEAR
	
	JSON_DATA_FILENAME ="/var/www/html/json/"+ENVIRONMENT.lower()+".errors.after.event.extraction.json" 
	print ("Start writing json data ...\n")

	
	#"legenddata": { "1": "Succeeded Errors", "2": "Failed Errors" }, \
	#"legendcolors": { "1": "lime", "2": "red" }, \		
	#"1": B_ERRORS_SUCCEEDED, \
	#"1": B_ERRORS_SUCCEEDED, \
	
	new_json_data = { \
		"charttitle": ""+ENVIRONMENT.lower()+".errors.after.event.extraction", \
		"charttype": "line", \
		"dotColor": "black", \
		"dotSize": "4", \
		"updateInterval": "5", \
