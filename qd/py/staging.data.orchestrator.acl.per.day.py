...
def initializeGlobalArrays():
	global ACLS_SUCCEEDED, ACLS_FAILED, B_ACLS_SUCCEEDED, B_ACLS_FAILED
	ACLS_SUCCEEDED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		ACLS_SUCCEEDED[i] = 0
	ACLS_FAILED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		ACLS_FAILED[i] = 0
	B_ACLS_SUCCEEDED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		B_ACLS_SUCCEEDED[i] = 0
	B_ACLS_FAILED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		B_ACLS_FAILED[i] = 0	
...
def runBaseDocFailed(environment):
	global B_ACLS_FAILED
	#LOGFILE = selectLogFile(environment,"ocr")
	LOGFILE = "summary_dataorchestrator_acl_staging"
	...
	#print new_result
		B_ACLS_FAILED[ind] = int(i[0])
		ind = ind + 1
	
	
def runBaseDocSucceeded(environment):
	global B_ACLS_SUCCEEDED
	#LOGFILE = selectLogFile(environment,"ocr")
	LOGFILE = "summary_dataorchestrator_acl_staging"
	...
	for i in new_result:
		...
		#print i
		#print cur.fetch()
		B_ACLS_SUCCEEDED[ind] = int(i[0])
		ind = ind + 1	
	
	
	
def runDocSucceeded(environment):
	global ACLS_SUCCEEDED
	#LOGFILE = selectLogFile(environment,"ocr")
	LOGFILE = "summary_dataorchestrator_acl_staging"
...
def runDocFailed(environment):
	...
	for i in new_result:
		print (str(i[0])+" \t\t "+str(i[1])+" \t\t "+str(i[2])+" \t\t "+str(i[3]))	
	#print new_result
		ACLS_FAILED[ind] = int(i[0])
		ind = ind + 1
	
def runQueries(environment):
	global ST_DAY, EN_DAY, MONTH, YEAR, ACLS_SUCCEEDED, ORGID, DAYSOFMONTH, ACLS_FAILED, B_ACLS_FAILED, B_ACLS_SUCCEEDED
	global ST_DAY, ST_MONTH, ST_MONTHS,ST_YEAR, EN_DAY, EN_MONTH, EN_MONTHS, EN_YEAR
...
def appendData():
	global DATA, ORGID, ENVIRONMENT, MONTH_FMN#, ACLS_FAILED, ACLS_SUCCEEDED, B_ACLS_FAILED, B_ACLS_SUCCEEDED
	global ST_DAY, ST_MONTH, ST_MONTHS,ST_YEAR, EN_DAY, EN_MONTH, EN_MONTHS, EN_YEAR
	
	JSON_DATA_FILENAME ="/var/www/html/json/"+ENVIRONMENT.lower()+".data.orchestrator.acl.per.day.json" 
	print ("Start writing json data ...\n")

	new_json_data = { \
		"charttitle": ""+ENVIRONMENT.lower()+".data.orchestrator.acl.per.day", \
		"charttype": "line", \
		"dotColor": "black", \
		"dotSize": "4", \
		"updateInterval": "5", \
		"legendtitle": {"1": "Legend"}, \
		"legenddata": { "1": "Succeeded ACLs", "2": "Failed ACLs" }, \
