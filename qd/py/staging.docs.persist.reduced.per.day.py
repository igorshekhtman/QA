...
def runBaseDocFailed(environment):
	...
	LOGFILE = "summary_persist_reducer_staging"
	...
	cur.execute("""SELECT COUNT(*) as  Persist_reducer_count, \
		year, month, day \
		FROM %s \
...
def appendData():
	...
	JSON_DATA_FILENAME ="/var/www/html/json/"+ENVIRONMENT.lower()+".docs.persist.reduced.per.day.json" 
	print ("Start writing json data ...\n")

	new_json_data = { \
		"charttitle": ""+ENVIRONMENT.lower()+".docs.persist.reduced.per.day", \
		"charttype": "line", \
		"dotColor": "black", \
		"dotSize": "4", \
		"updateInterval": "5", \
