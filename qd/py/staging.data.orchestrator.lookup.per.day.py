...
def initializeGlobalArrays():
	global LOOKUPS_SUCCEEDED, LOOKUPS_FAILED, B_LOOKUPS_SUCCEEDED, B_LOOKUPS_FAILED
	LOOKUPS_SUCCEEDED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		LOOKUPS_SUCCEEDED[i] = 0
	LOOKUPS_FAILED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		LOOKUPS_FAILED[i] = 0
	B_LOOKUPS_SUCCEEDED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		B_LOOKUPS_SUCCEEDED[i] = 0
	B_LOOKUPS_FAILED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		B_LOOKUPS_FAILED[i] = 0	
...
def runBaseDocFailed(environment):
	global B_LOOKUPS_FAILED
	#LOGFILE = selectLogFile(environment,"ocr")
	LOGFILE = "summary_dataorchestrator_lookup_staging"
	...
	#print new_result
		B_LOOKUPS_FAILED[ind] = int(i[0])
		ind = ind + 1
	
	
def runBaseDocSucceeded(environment):
	global B_LOOKUPS_SUCCEEDED
	#LOGFILE = selectLogFile(environment,"ocr")
	LOGFILE = "summary_dataorchestrator_lookup_staging"
	...
	for i in new_result:
		...
		#print i
		#print cur.fetch()
		B_LOOKUPS_SUCCEEDED[ind] = int(i[0])
		ind = ind + 1	
	
	
	
def runDocSucceeded(environment):
	global LOOKUPS_SUCCEEDED
	#LOGFILE = selectLogFile(environment,"ocr")
	LOGFILE = "summary_dataorchestrator_lookup_staging"
	...
	for i in new_result:
		...
		#print i
		#print cur.fetch()
		LOOKUPS_SUCCEEDED[ind] = int(i[0])
		ind = ind + 1	

def runDocFailed(environment):
	global LOOKUPS_FAILED
	#LOGFILE = selectLogFile(environment,"ocr")
	LOGFILE = "summary_dataorchestrator_lookup_staging"
	...
	for i in new_result:
		print (str(i[0])+" \t\t "+str(i[1])+" \t\t "+str(i[2])+" \t\t "+str(i[3]))	
	#print new_result
		LOOKUPS_FAILED[ind] = int(i[0])
		ind = ind + 1
	
def runQueries(environment):
	global ST_DAY, EN_DAY, MONTH, YEAR, LOOKUPS_SUCCEEDED, ORGID, DAYSOFMONTH, LOOKUPS_FAILED, B_LOOKUPS_FAILED, B_LOOKUPS_SUCCEEDED
	global ST_DAY, ST_MONTH, ST_MONTHS,ST_YEAR, EN_DAY, EN_MONTH, EN_MONTHS, EN_YEAR
...
def appendData():
	global DATA, ORGID, ENVIRONMENT, MONTH_FMN#, LOOKUPS_FAILED, LOOKUPS_SUCCEEDED, B_LOOKUPS_FAILED, B_LOOKUPS_SUCCEEDED
	global ST_DAY, ST_MONTH, ST_MONTHS,ST_YEAR, EN_DAY, EN_MONTH, EN_MONTHS, EN_YEAR
	
	JSON_DATA_FILENAME ="/var/www/html/json/"+ENVIRONMENT.lower()+".data.orchestrator.lookup.per.day.json" 
	print ("Start writing json data ...\n")

	new_json_data = { \
		"charttitle": ""+ENVIRONMENT.lower()+".data.orchestrator.lookup.per.day", \
		"charttype": "line", \
		"dotColor": "black", \
		"dotSize": "4", \
		"updateInterval": "5", \
		"legendtitle": {"1": "Legend"}, \
		"legenddata": { "1": "Succeeded Lookups", "2": "Failed Lookups" }, \
		"legendcolors": { "1": "lime", "2": "red" }, \
		"xaxistitle": str(DAYSBACK+1)+" Days", \
		"baselinedata": { \
		"1": B_LOOKUPS_SUCCEEDED, \
		"2": B_LOOKUPS_FAILED}, \
		"xaxisdata": { \
		"1": calcXAxis()}, \
		"yaxistitle": "# of Lookups",
		"yaxisdata": { \
		"1": LOOKUPS_SUCCEEDED, \
        "2": LOOKUPS_FAILED} \
		}	
