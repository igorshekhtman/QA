...
def initializeGlobalArrays():
	global OPPORTINITIES_SUCCEEDED, OPPORTINITIES_FAILED, B_OPPORTINITIES_SUCCEEDED, B_OPPORTINITIES_FAILED
	OPPORTINITIES_SUCCEEDED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		OPPORTINITIES_SUCCEEDED[i] = 0
	OPPORTINITIES_FAILED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		OPPORTINITIES_FAILED[i] = 0
	B_OPPORTINITIES_SUCCEEDED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		B_OPPORTINITIES_SUCCEEDED[i] = 0
	B_OPPORTINITIES_FAILED=[0 for i in range(DAYSBACK+1)]
	for i in range(DAYSBACK+1):
		B_OPPORTINITIES_FAILED[i] = 0	
...
def runBaseDocFailed(environment):
	...
	#print new_result
		B_OPPORTINITIES_FAILED[ind] = int(i[0])
		ind = ind + 1
	
	
def runBaseDocSucceeded(environment):
	global B_OPPORTINITIES_SUCCEEDED
	LOGFILE = selectLogFile(environment,"ocr")
	...
	for i in new_result:
		...
		#print i
		#print cur.fetch()
		B_OPPORTINITIES_SUCCEEDED[ind] = int(i[0])
		ind = ind + 1	
	
	
	
def runDocSucceeded(environment):
	global OPPORTINITIES_SUCCEEDED
	LOGFILE = selectLogFile(environment,"ocr")
	...
	for i in new_result:
		...
		#print i
		#print cur.fetch()
		OPPORTINITIES_SUCCEEDED[ind] = int(i[0])
		ind = ind + 1	

def runDocFailed(environment):
	global OPPORTINITIES_FAILED
	LOGFILE = selectLogFile(environment,"ocr")
	...
	for i in new_result:
		print (str(i[0])+" \t\t "+str(i[1])+" \t\t "+str(i[2])+" \t\t "+str(i[3]))	
	#print new_result
		OPPORTINITIES_FAILED[ind] = int(i[0])
		ind = ind + 1
	
def runQueries(environment):
	global ST_DAY, EN_DAY, MONTH, YEAR, OPPORTINITIES_SUCCEEDED, ORGID, DAYSOFMONTH, OPPORTINITIES_FAILED, B_OPPORTINITIES_FAILED, B_OPPORTINITIES_SUCCEEDED
	global ST_DAY, ST_MONTH, ST_MONTHS,ST_YEAR, EN_DAY, EN_MONTH, EN_MONTHS, EN_YEAR
...
def appendData():
	global DATA, ORGID, ENVIRONMENT, MONTH_FMN#, OPPORTINITIES_FAILED, OPPORTINITIES_SUCCEEDED, B_OPPORTINITIES_FAILED, B_OPPORTINITIES_SUCCEEDED
	global ST_DAY, ST_MONTH, ST_MONTHS,ST_YEAR, EN_DAY, EN_MONTH, EN_MONTHS, EN_YEAR
	
	JSON_DATA_FILENAME ="/var/www/html/json/"+ENVIRONMENT.lower()+".opportunities.last.day.json" 
	print ("Start writing json data ...\n")

	new_json_data = { \
		"charttitle": ""+ENVIRONMENT.lower()+".opportunities.last.day", \
		"charttype": "line", \
		"dotColor": "black", \
		"dotSize": "4", \
		"updateInterval": "5", \
		...
		"baselinedata": { \
		"1": B_OPPORTINITIES_SUCCEEDED, \
		"2": B_OPPORTINITIES_FAILED}, \
		"xaxisdata": { \
		"1": calcXAxis()}, \
		"yaxistitle": "# of Opportunities",
		"yaxisdata": { \
		"1": OPPORTINITIES_SUCCEEDED, \
        "2": OPPORTINITIES_FAILED} \
		}	
