...
def runBaseDocFailed(environment):
	...
	LOGFILE = "summary_bundler_sequence"+POSTFIX
	
	
	print ("Running %s Hive Query to extract failed historical baseline data, please wait ...\n") % (environment)

		
	cur.execute("""SELECT COUNT(*) as sequence_total, \
		year, month, day \
		FROM %s \
...
def appendData():
	...
	JSON_DATA_FILENAME ="/var/www/html/json/"+ENVIRONMENT.lower()+".bundler.sequence.per.day.json" 
	print ("Start writing json data ...\n")

	new_json_data = { \
		"charttitle": ""+ENVIRONMENT.lower()+".bundler.sequence.per.day", \
		"charttype": "line", \
		"dotColor": "black", \
		"dotSize": "4", \
		"updateInterval": "5", \
		"legendtitle": {"1": "Legend"}, \
		"legenddata": { "1": "Succeeded Sequence", "2": "Failed Sequence" }, \
