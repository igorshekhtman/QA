...
for i in range(60):
	DATA[i] = 0
ORGID=[0 for i in range(60)]
for i in range(60):
	ORGID[i] = 0
DAYSOFMONTH=[0 for i in range(60)]
for i in range(60):
	DAYSOFMONTH[i] = i+1
...
def appendData():
	global DATA, ORGID, ENVIRONMENT, MONTH_FMN, DAY, HOUR
	DATA_FILENAME ="/var/www/html/json/"+ENVIRONMENT.lower()+".opportunities.last.hour.json" 
	print ("Start writing json data ...\n")

	new_data = {\
		"xaxisdata": {"1": DAYSOFMONTH}, \
		"yaxisdata": {"1": DATA}, \
		"charttitle": ""+ENVIRONMENT.lower()+".opportunities.last.hour", \
		"yaxistitle": "Opportunities", \
		"xaxistitle": "Minute of the Hour" , \
		"legendtitle": {"1": "Opportunities per Minute - "+HOUR+" (hour of the day)"}, \
		"legenddata": {"1": "Opportunities"}, \
