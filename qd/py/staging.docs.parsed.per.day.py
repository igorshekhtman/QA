...
def runBaseDocFailed(environment):
	...
	cur.execute("""SELECT COUNT(DISTINCT get_json_object(line, '$.documentuuid')) as documents_parsed, \
		year, month, day \
		FROM %s \
...
def appendData():
	...
	JSON_DATA_FILENAME ="/var/www/html/json/"+ENVIRONMENT.lower()+".docs.parsed.per.day.json" 
	print ("Start writing json data ...\n")

	new_json_data = { \
		"charttitle": ""+ENVIRONMENT.lower()+".docs.parsed.per.day", \
		"charttype": "line", \
		"dotColor": "black", \
		"dotSize": "4", \
		"updateInterval": "5", \
