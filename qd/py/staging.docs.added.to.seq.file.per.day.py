...
def runBaseDocFailed(environment):
	...
	LOGFILE = "summary_docreceiver_seqfile_post_staging"
	...
	cur.execute("""SELECT SUM(num_docs) as documents_added_to_seq_file, \
		year, month, day \
		FROM %s \
...
def appendData():
	...
	JSON_DATA_FILENAME ="/var/www/html/json/"+ENVIRONMENT.lower()+".docs.added.to.seq.file.per.day.json" 
	print ("Start writing json data ...\n")

	new_json_data = { \
		"charttitle": ""+ENVIRONMENT.lower()+".docs.added.to.seq.file.per.day", \
		"charttype": "line", \
		"dotColor": "black", \
		"dotSize": "4", \
		"updateInterval": "5", \
