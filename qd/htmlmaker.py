...
def generateHTML(environment, JSON_LOC):
  filename = environment + '.' + JSON_LOC + '.html'
          ...
          graph.append("svg:svg").attr("id","currData");
                for ( numSets = 0; numSets < data.length; numSets++) {
                        for( numBars = 0; numBars < data[numSets].length; numBars++) {
                                d3.select("#currData")
                                        .append("rect").attr("width",barWidth).attr("height",h - y(dottedData[numSets][numBars])).attr("x",x(numBars) + (barWidth * (numSets*2+1))).attr("y",y(dottedData[numSets][numBars])).style("fill",le
gendColors[numSets]).style("opacity",0.3).on("mouseover",
                        function(d) {
            div.transition()
                .duration(50)
                .style("opacity", .9);
