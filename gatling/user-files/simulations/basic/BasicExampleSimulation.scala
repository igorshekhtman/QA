...
class BasicExampleSimulation extends Simulation {
  ...
  //val ip = "54.193.204.86"
  //val ip = "50.18.147.10"
  val ip = "10.191.15.150"
  ...
  val scn = scenario("HCC app under load")
    ...
    .group("Patient") {
      ...
      http("Patient demographics")
        ...
        .get("10.191.15.150:8085/patient/${patient_uuid}/demographics")
    ...
    .group("Document") {
      ...
      http("Document retrieval text")
        .get("10.191.15.150:8085/document/${document_uuid}/text")
        //.get(session => document + "/" + documentuuid + "/text")
        .header("Authorization", "Apixio ${internalToken}")
        .check(status.is(ok)))
      .exec( 
      http("Document retrieval metadata")
        .get("10.191.15.150:8085/document/${document_uuid}/metadata")
        //.get(session => document + "/" + documentuuid + "/metadata")
        .header("Authorization", "Apixio ${internalToken}")
        .check(status.is(ok)))
      .exec( 
      http("Document retrieval file")
        .get("10.191.15.150:8085/document/${document_uuid}/file")
  ...
  // 30 users over 5 min
  // setUp(scn.inject(ramp(30 users) over (300 seconds)))
