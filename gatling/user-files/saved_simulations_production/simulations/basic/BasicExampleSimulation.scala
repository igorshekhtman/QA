...
class BasicExampleSimulation extends Simulation {
  ...
  val scn = scenario("HCC app under load")
    ...
    .group("Login") {
      ...
      http("Login page")
        ...
        .check(status.is(ok)))
      .pause(thinkTime)
      .feed(csv("user_credentials_prod.csv"))
      .exec(
        http("Submit login")
          .post(loginSubmit)
    ...
    //.group("Patient") {
    //  feed(csv("patient_uuids_prod.csv"))
    //  .exec(
    ...
    //}
    //.group("Document") {
    //  feed(csv("document_uuids_prod.csv"))
    //  .exec( 
  ...
  //
  setUp(scn.inject(nothingFor(2 seconds), atOnce(5 users), ramp(50 users) over (5 minutes)))
