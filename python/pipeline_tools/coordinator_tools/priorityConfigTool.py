...
def validateUpdateString(input_string):
	delimiter = ','
  	...
  	elif (str(input_string[1].upper()) != "TRUE") and (str(input_string[1].upper()) != "FALSE"):
  		validation_string = "Only True or False is allowed for Enabled field. %s is not allowed, please re-enter ..." % input_string[1]
  	else:
...
def getListOfOrgs():
	...
	print ("----------------------------------------------------------------------------")
	print (">>> GET LIST OF ORGS AND PRIORITIES <<<")
	print ("----------------------------------------------------------------------------")

	url = PIPEHOST+"/pipeline/coord/orgs"
