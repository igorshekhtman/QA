...
translt = {'i': 'ignore', 'l': 'list', 'r': 'resubmit'}
...
def get_environment():
    ...
    if len(i) > 0 and i.upper()[0] == "Q":
        quit()
    if "P" in i.upper() or "PROD" in i.upper() or "PRD" in i.upper():
        return prod
    if "S" in i.upper() or "STAG" in i.upper() or "STG" in i.upper():
        return stag
...
def action_loop(actions, tool_name):
    ...
    while len(i) == 0 or i.upper()[0] != "Q":
        try:
            if len(i) == 1:
                print str(translt[i])
                actions[translt[i].lower().strip()](environment, token)
            else:
