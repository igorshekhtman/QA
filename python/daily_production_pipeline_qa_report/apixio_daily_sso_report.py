####################################################################################################
#
# PROGRAM: 	apixio_daily_sso_report.py
# AUTHOR:  	Igor Shekhtman ishekhtman@apixio.com
# DATE:    	2015.07.21 Initial Version
#
#
# PURPOSE:
...
REPORT_TYPE="Daily engineering SSO"
LOGTYPE = "epoch"
...
#-----------------------------------------------------------------------------------------
def obtainSSODetails(activity, summary_table_name, unique_id):
	global REPORT, cur, conn
	global DAY, MONTH, COMPONENT_STATUS
	...
	cur.execute("""SELECT count(*) as count, msg, error \
			FROM %s \
			WHERE \
			day=%s and month=%s and year=%s \
			GROUP BY msg, error \
...
def ssoRD():
	...
	REPORT = REPORT + SUBHDR % "SINGLE SIGN ON"
	COMPONENT_STATUS="PASSED"
	obtainSSODetails("Single Sign On", "summary_sso"+POSTFIX, "session")
...
def writeReportDetails():
	...
	if (REPSECTORUN == 11) or (REPSECTORUN == 0):
		loaderRD()	
	if (REPSECTORUN == 12) or (REPSECTORUN == 0):
		ssoRD()					
...
def archiveReport():
	...
	if not DEBUG_MODE:
		CURMONTH = MONTH[1:len(MONTH)-1]
		CURDAY = DAY[1:len(DAY)-1]
		BACKUPREPORTFOLDER="/mnt/reports/"+ENVIRONMENT+"/sso/"+str(YEAR)+"/"+str(CURMONTH)
		#REPORTFOLDER="/usr/lib/apx-reporting/html/assets/reports/"+ENVIRONMENT+"/pipeline/"+str(YEAR)+"/"+str(CURMONTH)
		REPORTFOLDER="/usr/lib/apx-reporting/assets/reports/"+ENVIRONMENT+"/sso/"+str(YEAR)+"/"+str(CURMONTH)
		...
		REPORTXTSTRING="Daily "+ENVIRONMENT[:1].upper()+ENVIRONMENT[1:].lower()+" Report - "+str(MONTH_FMN)+" "+str(CURDAY)+", "+str(YEAR)+"\t"+"reports/"+ENVIRONMENT+"/sso/"+str(YEAR)+"/"+str(CURMONTH)+"/"+REPORTFILENAME+"\n"
		REPORTXTFILENAME="sso_reports_"+ENVIRONMENT.lower()+".txt"		
