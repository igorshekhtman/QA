...
def uploadData():
	#UPLOAD_URL="%s/receiver/batch/%s/document/upload?category=dev" % (HOST, BATCHID);
	c = pycurl.Curl()
...
for FILE in FILES:
    ...
    # =============== Uploading Data ==============================================================
    #UPLOAD_URL="%s/receiver/batch/%s/document/upload?category=dev" % (HOST, DRBATCH)
    bufu = io.BytesIO()
...
CLOSE_URL="%s/receiver/batch/%s/status/flush?category=dev" % (HOST, DRBATCH);
bufc = io.BytesIO()
response = cStringIO.StringIO()
c = pycurl.Curl()
