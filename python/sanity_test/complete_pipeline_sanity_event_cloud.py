...
def initializeGlobalVars():
    ...
    DIR="/Users/ishekhtman/Git/QA/python/sanity_test/SanityThirteenDocuments/Documents"
...
def uploadFiles():
    ...
    for FILE in FILES:
        ...
        MANIFEST_FILE += ("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t02/22/14 02:02:37 AM\n") % (
...
def generateReportHeader():
    ...
    REPORT = ""
    REPORT += "<h1>Apixio Pipeline QA Report</h1>"
    REPORT += "Run date & time: <b>%s</b><br>\n" % (CUR_TIME)
    REPORT += "Report type: <b>%s</b><br>\n" % (REPORT_TYPE)
    REPORT += "Enviromnent: <b><font color='red'>%s%s</font></b><br>" % (
