####################################################################################################
#
# UAPROGRAM: opprouterloadunload.py
...
#          * From QA2 server (54.176.225.214) /mnt/automation/hcc folder enter "python2.7 opprouterloadunload.py"
#		   * note: opprouterloadunload.csv configuration file must coexist in same folder as opprouterloadunload.py
#
# USAGE:
#          * Set the global variables (CSV_CONFIG_FILE_PATH and CSV_CONFIG_FILE_NAME), see below
#          * Configure global parameters in opprouterloadunload.csv located in the same folder
#          * Results will be printed on Console screen as well as mailed via QA report
#		   *
#		   * python2.7 opprouterloadunload.py staging ishekhtman@apixio.com ishekhtman@apixio.com
#		   *
#
...
# DATE: 12-Dec-2014
#
# SPECIFICS: Introduced opprouterloadunload.csv file, which now contains all GLOBAL variables and their
...
#            CSV_CONFIG_FILE_NAME = "opprouterloadunload.csv"
...
#            Accept Reject or Skip an Opportunity.  It is defined in opprouterloadunload.csv file.  Possible
...
REPORT_EMAIL = ""
REPORT_TYPE = "Opp Router Load Unload Test"
...
PATIENTS_OPPS_SERVED = {}
DUPLICATE_PATIENTS_OPPS_SERVED = {}
...
def startCoding():
  ...
  global PERCENT_OF_SERVED, HCC, COUNT_OF_SERVED, PATIENTS_OPPS_SERVED, DUPLICATE_PATIENTS_OPPS_SERVED
...
def writeReportHeader ():
	...
	REPORT = """ """
	REPORT = REPORT + """<h1>Apixio Opp Router Load/Unload Test Report</h1>\n"""
...
def writeReportFooter():
	global REPORT, SORTED_PERCENT_OF_TARGET_HCC_SERVED, REPORT_EMAIL, DUPLICATE_PATIENTS_OPPS_SERVED
...
def archiveReport():
	...
	if not DEBUG_MODE:
		...
		BACKUPREPORTFOLDER="/mnt/reports/"+ENVIRONMENT+"/opprtrlu/"+str(YEAR)+"/"+str(CURMONTH)
		REPORTFOLDER="/usr/lib/apx-reporting/assets/reports/"+ENVIRONMENT+"/opprtrlu/"+str(YEAR)+"/"+str(CURMONTH)
		...
		REPORTXTSTRING="OppRtrLoadUnload "+ENVIRONMENT[:1].upper()+ENVIRONMENT[1:].lower()+" Report - "+str(MONTH_FMN)+" "+str(CURDAY)+", "+str(YEAR)+"\t"+"reports/"+ENVIRONMENT+"/opprtrlu/"+str(YEAR)+"/"+str(CURMONTH)+"/"+REPORTFILENAME+"\n"
		REPORTXTFILENAME="opprtrlu_reports_"+ENVIRONMENT.lower()+".txt"
		# Old location 
...
def emailReport():
	...
	message['Subject'] = 'OppRouter %s Load/Unload Test Report - %s\n\n' % (ENVIRONMENT, START_TIME)
