#=========================================================================================
#============================= energyroutingstatus.py ====================================
#=========================================================================================
#
# PROGRAM:         energyroutingstatus.py
...
#with optional batch id: ?batch_id={batchId} if you want to limit bundling to a particular run of events
	...
	print ("* Energy setting param.  = %s"%ENERGY_SETNG)
...
def checkEnvironmentandReceivers():
	...
	# Arg2 - on/off/status (true/false/status)
	...
	global ENERGY_RTR_URL, ENERGY_SETNG
	...
	if (len(sys.argv) > 2):
		ENERGY_SETNG=str(sys.argv[2])
	else:
		print "Missing Energy Setting paramater, aborting now ..."
		print "Proper use instructions:"
		print "python2.7 esbundle.py <environment> <energy-setting: true/false/none (required)>"
		quit()
...
def checkEnergyRouting():
	print ("\n----------------------------------------------------------------------------")
	print (">>> CHECKING ENERGY ROUTING FOR %s ENVIRONMENT <<<" % ENVIRONMENT)
	print ("----------------------------------------------------------------------------")
	response = ""
	if ENERGY_SETNG.upper() == "TRUE":
		URL = ENERGY_RTR_URL + "/true"
	elif ENERGY_SETNG.upper() == "FALSE":
		URL = ENERGY_RTR_URL + "/false"
	else:
		URL = ENERGY_RTR_URL
	...
	print ("\n")
	print ("* URL                    = %s" % URL)
	print ("* Energy setting flag    = %s" % ENERGY_SETNG)
	print ("* Environment            = %s" % ENVIRONMENT)
	...
	if ENERGY_SETNG.upper() == "TRUE":
		...
		print ("* Energy Routing Status  = %s" % response.json().get("energyRouting"))
	elif ENERGY_SETNG.upper() == "FALSE":
		response = requests.put(URL, data=json.dumps(DATA), headers=HEADERS)
		print ("* Status Code            = %s" % response.status_code)
		print ("* Energy Routing Status  = %s" % response.json().get("energyRouting"))	
	...
	if response.status_code != ok:
		...
		print "\nSuccessfully obtaining/setting Energy Routing status ..."		
...
print ("\n============================================================================")	
print ("===================== End of Energy Routing Status =========================")
print ("============================================================================")
#=========================================================================================
