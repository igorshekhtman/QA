...
def getAction(opportunity, coder, state):
  ...
  initial_state = ""
  #for aplan in aplans:
  #  if initial_state != aplan.get("state"):
  #    print aplan.get("state")
  #    initial_state = aplan.get("state")
  #quit() 
  ...
  coders = ["protest01@apixio.net", "mmgenergyes@apixio.net", "ishekhtman@apixio.com", "restest07@apixio.net"]
  pw = "apixio.123"
  ...
  actions_chart = { \
		"accepted_code":      [1, 1, 1], \
		"rejected_code":      [2, 2, 2], \
		"rejected_code_more": [2, 2, 2], \
		"accepted_qa1":       [1, 1, 1], \
		"rejected_qa1":       [2, 2, 2], \
		"rejected_qa1_more":  [2, 2, 2], \
		"accepted_qa2":       [1, 1, 1], \
		"rejected_qa2":       [2, 2, 2], \
		"rejected_qa2_more":  [2, 2, 2], \
		"accepted_qa3":       [1, 1, 1], \
		"rejected_qa3":       [2, 2, 2], \
		"rejected_qa3_more":  [2, 2, 2]  \
		}
...
def startCoding():
  ...
  print("-------------------------------------------------------------------------------")
  print("* URL                = %s" % URL)
  print("* CODER USERNAME     = %s" % USERNAME)
  print("* CODER PASSWORD     = %s" % PASSWORD)
  ...
  for coding_opp_current in range(1, (int(CODE_OPPS_MAX)+1)):
    ...
    for finding in findings:
      ...
      act_on_doc(opportunity, finding, finding_id, testCode + test_counter, doc_no_current, doc_no_max, getAction(opportunity, coder, "accepted_qa1"))
...
#coders = ["mmgenergyes@apixio.net", "protest01@apixio.net", "ishekhtman@apixio.com", "restest07@apixio.net"]
coders = ["mmgenergyes@apixio.net"]
pw = "apixio.123"
states = [ \
		"Code Accept", "Code Reject", "Code Reject with moreFindings", \
		"QA1 Accept", "QA1 Reject", "QA1 Reject with moreFindings", \
		"QA2 Accept", "QA2 Reject", "QA2 Reject with moreFindings", \
		"QA3 Accept", "QA3 Reject", "QA3 Reject with moreFindings"	\
		]
