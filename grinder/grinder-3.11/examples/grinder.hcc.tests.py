...
def code():
  ...
  for coding_opp_current in range(1, (int(CODE_OPPS_MAX)+1)):
    ...
    opp_get_time = current_end_time - current_begin_time
    ...
    log("-------------------------------------------------------------------------------")
    log("PATIENT OPP %d OF %d (opp get time < %d second(s))" % (coding_opp_current, int(CODE_OPPS_MAX), (opp_get_time + 1)))
    test_counter = 0
    doc_no_current = 0
